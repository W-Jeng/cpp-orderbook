...
static void BM_SingleThreadedOrderBookInsert(benchmark::State& state) {
    const size_t NUM_ORDERS = state.range(0);
    const size_t NUM_INSTRUMENTS = state.range(1);
...
BENCHMARK(BM_SingleThreadedOrderBookInsert) 
    ->Args({1'000, 1})
    ->Args({100'000, 1})
    ->Args({1'000'000, 1});
